 ...
 * these stay valid when S3 mirrors Y.
 */
...
const RUNS: Array<["cut" | "fill", PowerMode]> = [
  ["cut", "M3"],
  ["cut", "M4"],
  ["fill", "M3"],
  ["fill", "M4"],
...
describe.each(RUNS)("material test program (mode=%s, powerMode=%s)", (mode, powerMode) => {
  const opts = makeOpts(mode, powerMode);
  const cells = opts.powerSteps * opts.speedSteps;
  const program = () => generateMaterialTestGcode(opts, S_MAX);
  const parsed = async () =>
    (await program())
      .split("\n")
      .map(parseLine)
...
describe("frame trace", () => {
  it("F1: generateFrameGcode has no M3/M4 word and no S word", () => {
    const lines = generateFrameGcode(30, 20)
      .split("\n")
      .map(parseLine)
      .filter((w): w is Words => w !== null);
