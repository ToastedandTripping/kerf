  ...
  while (i < lines.length) {
    ...
    if (code === 0 && value === "LINE") {
      ...
      newObjects.push({
        id: generateId(),
        type: "line",
        name: `DXF Line ${newObjects.length + 1}`,
        transform: {
          x: Math.min(x1, x2),
          y: Math.min(y1, y2),
      ...
      newObjects.push({
        id: generateId(),
        type: "ellipse",
        name: `DXF Circle ${newObjects.length + 1}`,
        transform: {
          x: cx - r,
          y: cy - r,
          width: r * 2,
      ...
      if (pts.length >= 2) {
        ...
        newObjects.push({
          id: generateId(),
          type: "path",
          name: `DXF Polyline ${newObjects.length + 1}`,
          transform: {
            x: bb.x,
            y: bb.y,
            width: bb.width,
      ...
      if (r > 0 && startAngle !== endAngle) {
        ...
        if (pts.length >= 2) {
          ...
          newObjects.push({
            id: generateId(),
            type: "path",
            name: `DXF Arc ${newObjects.length + 1}`,
            transform: {
              x: bb.x,
              y: bb.y,
              width: bb.width,
