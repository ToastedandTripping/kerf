...
function parseTransformAttr(attr: string): Matrix {
  ...
  for (const t of transforms) {
    ...
    switch (name) {
      ...
      case "scale": {
        const sx = nums[0] || 1;
        m = [sx, 0, 0, nums.length > 1 ? nums[1] : sx, 0, 0];
        break;
      }
      case "rotate": {
...
export function SvgImportDialog({ open, svgContent, onClose }: Props) {
  ...
  function setLayerForColor(colorIndex: number, layerIndex: number) {
    setColorGroups((prev) => prev.map((g, i) => (i === colorIndex ? { ...g, layerIndex } : g)));
  }
    ...
    <>
      ...
      >
        <div style={{ padding: "20px 20px 0" }}>
          ...
          {textFonts.length > 0 && (
            <div
              style={{
                marginBottom: "12px",
                padding: "8px 10px",
                background: "rgba(226,200,74,0.1)",
                border: "1px solid rgba(226,200,74,0.3)",
                borderRadius: "var(--radius-sm)",
                fontSize: "11px",
                color: "rgba(226,200,74,0.9)",
              }}
            >
              This SVG contains text using: <strong>{textFonts.join(", ")}</strong>. Text may render
              differently without the original fonts. Convert text to paths before exporting for
              exact results.
            </div>
          )}
          ...
          {colorGroups.length > 0 && (
            <div style={{ marginBottom: "12px" }}>
              ...
              >
                Assign colors to layers ({colorGroups.length} color
                {colorGroups.length !== 1 ? "s" : ""} found)
              </div>
              <div style={{ fontSize: "10px", color: "var(--text-muted)", marginBottom: "8px" }}>
                Each color group maps to a layer. Elements with the same color will share
              ...
              <div style={{ maxHeight: "200px", overflowY: "auto" }}>
                {colorGroups.map((group, idx) => (
                  ...
                  >
                    ...
                    />
                    <span
                      style={{ fontSize: "12px", color: "var(--text-muted)", minWidth: "60px" }}
                    >
                      {group.color}
                    </span>
                    <span
                      style={{ fontSize: "11px", color: "var(--text-muted)", minWidth: "30px" }}
                    >
                      x{group.count}
                    </span>
                    <div style={{ flex: 1 }} />
        ...
        >
          ...
          >
            Select any object and use the Layer dropdown in Properties to reassign it, or press 1-6
            to assign layers. Each element can have its own layer for separate cut/engrave settings.
          </div>
        </div>
        ...
        >
          ...
          >
            {importing ? "Importing..." : "Import with Layers"}
          </button>
        </div>
      </div>
    </>
...
function importSvgWithLayers(svgText: string, colorToLayer: Map<string, number> | null) {
  ...
  const initialMatrix: Matrix =
    vbOffset.x !== 0 || vbOffset.y !== 0 ? [1, 0, 0, 1, -vbOffset.x, -vbOffset.y] : identityMatrix;

  const skippedElements = new Map<string, number>();
  walkElementWithLayers(
    svg,
    initialMatrix,
    globalScale,
    styleMap,
...
function parseSvgElementForImport(
  ...
  const base: Omit<DesignObject, "type" | "transform"> = {
    ...
    strokeWidth: Math.max(0.5, parseFloat(strokeWidthStr) * scale),
    opacity,
  };
  ...
  switch (tag) {
    case "rect": {
      ...
      const corners = [
        ...
        [x + w, y + h],
        [x, y + h],
      ].map(([px, py]) => applyMatrix2x3(matrix, px, py));
      if (isRotated) {
    ...
    case "circle": {
      const cx = n(el, "cx"),
        ...
        r = n(el, "r");
      if (r === 0) return null;
      const center = applyMatrix2x3(matrix, cx, cy);
      const ms = getMatrixScale(matrix);
      const rrx = r * ms.sx * scale,
        rry = r * ms.sy * scale;
      return {
        ...base,
        type: "ellipse",
        transform: {
          x: center.x * scale - rrx,
          y: center.y * scale - rry,
          width: rrx * 2,
          height: rry * 2,
          rotation: 0,
          scaleX: 1,
    ...
    case "ellipse": {
      ...
      if (isRotated) {
        ...
        const cosR = Math.cos(matrixRotation),
          sinR = Math.sin(matrixRotation);
        const ax = erx * ms.sx * scale * cosR,
          ay = erx * ms.sx * scale * sinR;
        const bx = -ery * ms.sy * scale * sinR,
          by = ery * ms.sy * scale * cosR;
        ...
        const pts = [
          {
            x: p0.x,
            y: p0.y,
            handleIn: { x: p0.x - K * bx, y: p0.y - K * by },
            handleOut: { x: p0.x + K * bx, y: p0.y + K * by },
          },
          {
            x: p1.x,
            y: p1.y,
            handleIn: { x: p1.x + K * ax, y: p1.y + K * ay },
            handleOut: { x: p1.x - K * ax, y: p1.y - K * ay },
          },
          {
            x: p2.x,
            y: p2.y,
            handleIn: { x: p2.x + K * bx, y: p2.y + K * by },
            handleOut: { x: p2.x - K * bx, y: p2.y - K * by },
          },
          {
            x: p3.x,
            y: p3.y,
            handleIn: { x: p3.x - K * ax, y: p3.y - K * ay },
            handleOut: { x: p3.x + K * ax, y: p3.y + K * ay },
          },
        ];
        const bb = boundingBox(pts);
        return {
    ...
    case "path": {
      ...
      for (const sub of parsePathD(d)) {
        ...
        const points: PathPoint[] = sub.points.map((p) => {
          ...
          if (p.handleIn) {
            const hi = applyMatrix2x3(matrix, p.handleIn.x, p.handleIn.y);
            result.handleIn = { x: hi.x * scale, y: hi.y * scale };
          }
          if (p.handleOut) {
            const ho = applyMatrix2x3(matrix, p.handleOut.x, p.handleOut.y);
    ...
    case "text": {
      ...
      const fontFamily = (getResolvedStyle(el, "font-family", styleMap) || "sans-serif").replace(
        /['"]/g,
        ""
      );
      const pos = applyMatrix2x3(matrix, x, y);
      const ms = getMatrixScale(matrix);
      fontSize = fontSize * ms.sy * scale;
      const estWidth = textContent.length * fontSize * 0.55,
        estHeight = fontSize * 1.3;
      return {
        ...base,
        type: "text",
        ...
        fontSize,
        fontFamily,
        fill: resolvedFill || resolvedStroke,
      };
    }
    default:
      return null;
