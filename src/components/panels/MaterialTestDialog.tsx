...
import {
  ...
  type PowerMode,
} from "../../lib/machine/materialTestGcode";
      ...
      navigator.clipboard
        ...
        .then(() => {
          store.addConsoleLine(
            `Material test G-code copied (${gcode.split("\n").length} lines)`,
            "info"
          );
        })
        .catch(() => {
        ...
        <div style={{ marginBottom: "12px" }}>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            ...
            <input
              type="number"
              min="1"
              max={effectiveSpeedMax}
              value={speedMin}
              onChange={(e) =>
                setSpeedMin(Math.max(1, Math.min(effectiveSpeedMax, Number(e.target.value))))
              }
              style={inputStyle}
            />
            <span style={labelStyle}>Max</span>
            <input
              ...
              value={speedMax}
              onChange={(e) =>
                setSpeedMax(Math.max(1, Math.min(effectiveSpeedMax, Number(e.target.value))))
              }
              style={inputStyle}
            />
          </div>
        ...
        <div style={{ display: "flex", gap: "12px", marginBottom: "12px" }}>
          ...
          >
            <input
              type="checkbox"
              checked={cutBorder}
              onChange={(e) => setCutBorder(e.target.checked)}
            />
            Cut border
          </label>
        </div>
