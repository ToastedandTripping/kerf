  ...
  function handleImport() {
    ...
    const obj: DesignObject = {
      ...
      transform: {
        x: 10,
        y: 10,
        width: effectiveWidthMm,
        height: effectiveHeightMm,
        rotation: 0,
        scaleX: 1,
        scaleY: 1,
      },
    ...
    useStore
      .getState()
      .setStatusMessage(
        `Imported ${fileName}${imageWidth > 0 ? ` -- ${imageWidth}x${imageHeight}px` : ""} -- ${effectiveWidthMm.toFixed(0)}x${effectiveHeightMm.toFixed(0)}mm`
      );
    ...
    <>
      ...
      >
        ...
        <div style={{ fontSize: "11px", color: "var(--text-muted)", marginBottom: "12px" }}>
          {fileName}
          {imageWidth > 0 ? ` -- ${imageWidth}x${imageHeight}px` : ""} (
          {effectiveWidthMm.toFixed(0)}x{effectiveHeightMm.toFixed(0)}mm)
        </div>
        {!widthMmOverride && (
        ...
        {showTraceTip && (
          ...
          >
            <span style={{ flex: 1 }}>
              Tip: For text and logos, use Trace (Alt+T) to convert to vectors for faster cutting.
              Raster engraving scans every pixel — best for photos and detailed images.
            </span>
            <button
              onClick={dismissTip}
        ...
        >
          After tracing, select any path and use the Layer dropdown in Properties to reassign it, or
          press 1-6. Each traced element can be on its own layer for separate cut/engrave settings.
        </div>
