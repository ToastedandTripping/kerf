...
pub fn generate_gcode(objects: &[CutObject], workspace_height: f64, s_value_max: f64, origin_top: bool) -> GcodeResult {
    ...
    // Footer
    // NOTE: assembleGcode() in gcodeGen.ts strips everything from
    // "; KERF:FOOTER_BEGIN" onward when merging multi-layer fragments.
    // If you change any footer line, update stripFraming() in gcodeGen.ts.
    lines.push("; KERF:FOOTER_BEGIN".to_string());
    lines.push("M5 ; laser off".to_string());
