...
**Citations:** re-read at `8c60666` on session branch `marvin/kerf-gap` (2026-09-25). S1 is merged there (`df8883a`). S2 is not merged: its plan is `.claude/plans/kerf-safety-s2.md` and no `relay/kerf-safety-s2` branch exists yet.

- **Relay id:** `kerf-safety-s3`
- **Branch:** `relay/kerf-safety-s3`
- **Tier:** Standard. 12 files in one root (`src/`), over the 8-file cap; waiver below. `.tsx` is touched, so Jen does a CONCERN pass with no spec.
...
- A jog moves only when Kerf knows where the head is and where the bed ends. Unknown bed, stale status, a moving head, a work offset it cannot account for, or a jog still in flight all refuse, in plain words, with nothing sent.
...
If S2 did not move the generators (it chose to export in place), S3 edits `MaterialTestDialog.tsx` for both the mirror and the call sites, the file count drops to 11, and the materialTestGcode anchors below move to that file. If S2 has not merged when S3 is due, S3 does not dispatch.

## Existing plans reviewed

**Inventory, checked with `ls` on 2026-09-25:** `.claude/plans/` holds 39 `.md` files plus `archive/` (3 files). The audit directory holds 9 files.
...
- `kerf-safety-s2.md`: the generator and its test, as above. S3 follows it.
...
- `refresh-canvas-display.md` (branch 9 commits ahead): lists `connection.ts` and `MachinePanel` as untouched. `git diff --name-only` of that branch against its merge base touches none of S3's 12 files.
- `refresh-editing-shortcuts.md` and S5 edit `toolHandler.ts`, which S3 does not edit.
...
| Parent S3 file list (6 files) | **No longer holds.** 12 files (see Waiver). |
...
| `toolHandler.ts:1807-1812` sends `workspaceHeight - worldY` | Holds (`:1807-1813`). |
...
4. A settings write since the last full parse. The key is nulled on every settings write (S1's `invalidateGrblSettingsSilently`), so a confirm made before the next full parse is session-only.
...
- `jogBlockReason(s: JogGateState, mode: "by" | "to"): string | null`. `JogGateState` holds the scalars `machineConnected`, `machineState`, `statusStale`, `workspaceVerified`, `positionKind`, `workCoordOffset`, so `useStore.getState()` satisfies it structurally. Checks, in order:
  1. Not connected: "Machine not connected".
  2. `if (s.machineState === "alarm") return JOG_REASON_ALARM;`
  3. `if (!s.workspaceVerified) return JOG_REASON_BED;`
  4. `if (s.statusStale || s.positionKind === null) return JOG_REASON_STALE;`
  5. `if (s.machineState !== "idle") return JOG_REASON_BUSY;`
  6. `const offsetSet = s.workCoordOffset.x !== 0 || s.workCoordOffset.y !== 0;`, then `if (offsetSet && (mode === "to" || s.positionKind === "work")) return JOG_REASON_OFFSET;`. A relative jog on an MPos report is exact whatever the offset. An absolute `$J=G90` target is in work coordinates, so any offset refuses it.
  7. `return null;`.
...
These are the only edits: `jog`, `jogTo`, a new module helper `sendJogLine`, one hook in `pollStatus`, the bed-memory helpers and their call sites in `parseSettingsResponses`, `connect` (one line), `disconnect`, `invalidateGrblSettingsSilently` (one line), and a new `confirmBedSize`.
...
- `jog(axis, distance, feedRate = 1000)`:
  ...
  - Normalize `axis` to `"X" | "Y"`. Call `clipJog` with `machinePosition.x`/`.y`, `workspaceWidth`/`workspaceHeight` and `originTop`. A refusal prints its reason and returns.
...
- `pollStatus`, beside E3's spindle-drop block:
  ...
  - After `consumeStatusOutcome`: `const noJogOverlap = tickAtInvoke === jogTick;`, then `if (accepted && jogPending && noJogOverlap && useStore.getState().machineState === "idle") jogPending = false;`.
  - A poll that began after the jog was acknowledged and reports Idle proves the jog finished, or never moved.
...
- `connect`: one line after `store.setMachineConnected(true)` (`:277`): `connectedPort = portName;`.
- `disconnect`, beside S1's laser-mode clear (`:411`): `store.setWorkspaceVerified(false);`, `connectedPort = null`, the bed key cleared, `markBed(null)` and `jogPending = false`. The bed key is cleared through a helper (`forgetConnectionBed()`), so the literal `bedKey = null;` in §invalidate stays unique.
- `invalidateGrblSettingsSilently` (`:116-119`) gains `bedKey = null; // S3: a confirm after a settings write is session-only`.
...
**Out of bounds for this batch:** `send()`, `emergencyStop`, `softReset`, `feedHold`, `home`, `setOrigin`, `enableLaserMode`, `readbackGrblSettings`, the `FS:` block, `applyLaserModeReadback`, `isGrblSettingsWrite`.
...
- **Jog buttons:**
  - Add scalar selectors for `statusStale` and `positionKind` (never an object selector).
  - `const jogBlocked = jogBlockReason(` with `{ machineConnected, machineState, statusStale, workspaceVerified, positionKind, workCoordOffset }` and `"by"`.
...
- `test_command` is `["npx","vitest","run","--cache=false","src/lib/machine/__tests__/jogBounds.test.ts","src/lib/machine/__tests__/connection.test.ts","src/lib/machine/__tests__/machineSafety.test.ts","src/lib/machine/__tests__/materialTestGcode.test.ts","src/components/panels/__tests__/machineJobLoop.test.tsx"]`. The `--cache=false` is carried from S1, whose plan-exact command errored 18/18 on a vitest cache write (S1 pack). The baseline must be green.
...
- **Anchor uniqueness.** Each mutant is one contiguous find/replace (`mutation-battery.mjs:1553-1560`). Every `find` except S3-M15's is new text this batch writes. The 27 new-code ids use 24 distinct `find` texts (S3-M19/S3-C2 and S3-M6/S3-C1 share a `find`; S3-M24/S3-M25 use the same text in different files). `grep -rF` of each of the 24 against `src/` at `8c60666` returns **0**, so nothing pre-existing collides. `\|` in the table is Markdown escaping for a literal `|`. S3-M15's `find` is existing text, and `grep -cF` in `canStartJob.ts` returns **1**. Prettier-sensitive anchors are written as single statements under 100 columns, or as a fragment that begins a statement. Ted re-runs `grep -cF` per file on the finished tree (each must be 1) and records the counts before the battery runs.
...
| S3-M14 | `connection.ts` | A `pollStatus` whose invoke is deferred starts; a `jog` is sent and acknowledged; the poll then resolves Idle; a second `jog` still makes no second send. A poll begun after the acknowledgement that reports Idle does clear it (positive half). | `const noJogOverlap = tickAtInvoke === jogTick;` → `const noJogOverlap = true;` |
...
| S3-M20 | `connection.ts` | Connect (key F); console `send("$100=80")` (a settings write, same value); `confirmBedSize(300, 200)`; disconnect; reconnect with F: **not** verified (the confirm was session-only) | `bedKey = null; // S3: a confirm after a settings write is session-only` → `` |
...
That is 28 ids: 25 mutants and 3 controls. The existing jog tests in `machineSafety.test.ts` (alarm refusal, `X10`/`X-5` clips, no send at the edge, verified clip) stay in the command as regressions, and so do S2's invariants over both frames.
...
- **Position Laser on an origin-top machine.** `toolHandler.ts:1811` still sends `workspaceHeight - worldY`, which is positive. S3's `jogTo` now refuses it instead of driving past home. S5 fixes the mapping.
...
1. **A verified bed can be resized by a project file or Settings.** `loadProject` and SettingsDialog write the workspace size while `workspaceVerified` stays true. Jog and START bounds then use an unverified size. Recommend S4b.
...
4. **Set Origin writes a zero work offset locally** (`connection.ts:656`) after `G92 X0 Y0` has made a non-zero one. The offset refusal trusts that field until the next `WCO:` report. It is a precondition for S5, when Position Laser can reach `jogTo` on origin-top machines.
...
- A stale status is displayed as "Stale".
...
## Waiver: 12 files, one root

The cap is 8. The file count is 12:
- 4 new or moved code/test files: `jogBounds.ts`, `jogBounds.test.ts`, and S2's `materialTestGcode.ts` and its test.
- 4 test files: `connection.test.ts`, `machineSafety.test.ts` (forced: it pins the removed raw send), `machineJobLoop.test.tsx` (forced: S1-M7 pins the removed refusal) and `materialTestGcode.test.ts`.
- 4 production edits beyond `connection.ts`: `MaterialTestDialog.tsx` (three call sites), `MachinePanel.tsx`, `StatusBar.tsx` and `machineStateDisplay.ts`.
...
- S3a: jog, bed memory and C3 (9 files).
- S3b: the mirror (4 files, sharing `machineJobLoop.test.tsx`).

Both would have to merge before any build. The critic may prefer it. Nothing in the design changes if it is split.

## Risks and rollback

**Operator-visible changes (intended):**
- Jogs refuse on an unconfirmed bed (`kerf-f1`), on a stale status, while moving, with an offset on a WPos report, and while the previous jog is in flight. Each shows a plain reason in the panel or console.
- Rapid clicks drop instead of queuing.
- Position Laser refuses on origin-top machines until S5.
...
- **Position Laser between S3 and S5.** A build carrying S3 without S5 refuses Position Laser on origin-top machines, where today it drives past home. That is safer, but it is a feature gap. The recommendation to the coordinator: no build is cut between S3 and S5, alongside the parent's rule that S1 and S3 share a build.
