...
- **A failed re-send closes admission against every later Idle store (critic B1).**
  ...
  - TypeScript maps `refused: in-flight-unreset` to endState `"unknown"`, not `"cancelled"`. That keeps `jobRunning` true, keeps the STOP button live for a retry through the shared stop, and sets `machineState` to alarm.
...
5. **Stop changes (`serial_stop_inner`), limited to the result-construction phase.** The `0x18` sequence itself is untouched.
   ...
   - **Result construction:**
     - (a) If `resend_failed` is set, do not transition to Idle. Return `SubmittedUnconfirmed { in_flight_write: true, … }` with the message "one job line reached the controller around STOP and the reset re-send failed; use the machine's physical stop. Beam state unqualified." This holds even when `banner_observed` is set. The physical-stop message comes **only** from this branch.
     - (b) Otherwise, when the banner was observed, emit `stop_confirming` and then call `set_idle_from_stopping()`. This is a `compare_exchange(PHASE_STOPPING, PHASE_IDLE)`.
       - If the CAS succeeds, increment the epoch and return `Confirmed`.
   ...
   - `serial_job_end_inner`'s Active→Idle store is unaffected. It requires `admitted_job == id`, and every stop clears `admitted_job`, so it cannot overwrite Unknown.
...
3. `ScriptedPort` gains two test-only faults, both `#[cfg(any(test, feature = "sim"))]`:
   - `hold_next_write(role, id)`: the next write from that role parks until `release_hold(id)` and is recorded in the trace at release. The brain lock is not held while parked; this is the same pattern as the read hold.
   - `fail_writes_after(role, n)`: that role's writes after the n-th return an I/O error.
...
|---|---|---|---|---|---|---|
| `refused: not-admitted: …` | No | `cancelled` | false | **No**: no `emergencyStop`, no M5, no line, whatever `jobRunning` says | No | `<label> stopped: the controller is no longer accepting this job's lines (<phase or epoch reason>). Nothing further was sent.` |
...
| `refused: in-flight-unreset: …` | Possibly one line; the reset re-send failed | **`unknown`** | **stays true**, so STOP stays live (`JobActionBar.tsx:296`) | **No** automatic stop; the operator can press STOP, which goes through the shared `serial_stop` | No | `<label> stopped: one line may have reached the controller after STOP and the reset could not be re-sent. Use the machine's physical stop, then press STOP. Beam state unqualified.` It also calls `setMachineState("alarm")`, mirroring `emergencyStop`'s `submissionFailed` handling. |
   ...
   - **The in-flight-unreset case.** The backend has already closed admission (F1, B1). The operator keeps the STOP button, and the retry goes through the same shared stop.
...
- **Harness** (`serialTraceHarness.ts`): add `enableFenceModel()`.
  - `serial_job_begin` admits the returned epoch; `serial_job_end` clears it; `serial_stop` closes admission and resolves a `confirmed` `StopResult` fixture; a `simulateReconnect()` helper clears the admission.
...
- **T4b (critic B1): `Ok("refused: in-flight-unreset: …")`, buffered and per-line (raw-string rejection).**
  - Assert: `endState === "unknown"`, `jobRunning === true`, `machineState === "alarm"`, zero `serial_stop` from TS, and the physical-stop line is present.
  - Mutant: map it to `"cancelled"` → **RED**.
...
- **Unfenced non-job writes during Stopping and Unknown.** `None`-epoch sends are not gated on phase, by design. Consequences:
  ...
  - Parked as "1.x follow-up: phase policy for manual writes".
...
- **New tests green and proven:** R1-R8b, F1's unit additions, F7, T1-T7 (including T4b and the harness self-test).
