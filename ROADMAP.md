---
status: active
current: v0.8.9 — two field-reported bug fixes (relay kerf-v0.8.9-bugfixes; Razor PASS, 487 JS tests). (1) SAFETY: the FIRST Start streamed into a GRBL ALARM-locked controller — homing ($22=1) boots to Alarm, connect() assumed "idle", and the Start gate never checked alarm, so the preamble (M3 laser-enable + dwell) energized the laser at the PARKED head before the first move was refused. Now Start/Frame REFUSE when alarmed ("Home ($H)/Unlock ($X) first") and connect() reads the real machine state (bounded in Rust, non-fatal); it never auto-homes/unlocks (user decision). (2) UI: dragging a power/speed slider escalated into the layer-row native drag-and-drop (the "dragging like an image" symptom) — drags starting on a form control no longer trigger reorder. ALSO this session: AUDITED the resize/rotation UX (owner: "not functioning as intended") and CONFIRMED it broken — rotated-object resize scales in SCREEN axes (handles sit on the screen-axis AABB, drag delta applied in screen space then re-rotated), so the handle doesn't track the cursor and the shape shears; and there's zero hover/rotate cursor feedback on handles. Data is correct (preview==cut) but the interaction is wrong. The resize/rotation rework is now top priority (see next). Base: v0.8.8 (even-engraving M4).
next: Resize/rotation rework (TOP — owner-reported broken, audit-confirmed): (a) local-axis rotated resize — draw + hit-test handles on the ROTATED rectangle and apply the drag delta in the object's local frame (the headline fix; currently screen-axis, toolHandler.ts:589-648 + Viewport handle render); (b) hover + directional/rotate cursors on handles (cheap, high felt-impact; hitTestHandle only runs on pointer-down today); (c) live rotation-angle readout + multi-select orbit-about-group-center + finer snap. Then Fill Phase C — Fix-2 winding hygiene; deferred — cross-hatch (B4, only if a post-M4 re-burn still shows grain) + offsetFill/Clipper2 (no new dep yet); then v0.9 — Camera & Rotary. Carry-overs: extract the Rust fill-before-line partition to a shared pub(crate) fn; fillLine-perimeter kerfOffset decision; per-machine scanningOffset calibration. (Retired stale note: group resize DOES propagate to children now — commit 12c3013.)
testing: null
pinned: true
shipped:
  - date: 2026-06-21
