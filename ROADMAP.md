...
- **Engine arms every real job with positive S on standalone mode lines (S2 Deferral 1): X1-class on every M3 layer.** A stationary beam at every path start on any layer set to M3. It needs its own batch, with golden-regeneration discipline; raised to the coordinator as a gap in the parent plan. See `### Deferred from kerf-safety-s2 (2026-09-25)` below.
