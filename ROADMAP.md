---
status: active
current: "v0.8.21 released + image-trace artifact fix (this session, 2026-06-28, relay kerf-trace-artifacts). (1) RELEASED v0.8.21: the post-v0.8.20 first-Start-freeze + SVG implicit-lineto fixes were on master but never tagged/built — the owner was still running v0.8.20 installers without them ('first fire still broken'). Cut v0.8.21 (CI-built .dmg/.AppImage/.deb, auto-published) and corrected version drift (Cargo.lock + package-lock.json had fallen behind to 0.8.18; all five version files now agree). (2) IMAGE-TRACE ARTIFACTS root-caused + fixed: clean black-on-white PNG line drawings traced with spurious interior 'shading'/small cuts. THREE bugs — interior WHITE pinholes were never despeckled (remove_small_components cleaned only black blobs; in vtracer Binary mode each pinhole rides along as a hole subpath in the cluster's compound path → a real cut); the Step-8 output filter judged contours by SVG-string LENGTH not geometric area; and the dialog preview traced at 0.25 scale, systematically cleaner than the 1.0 commit (artifacts invisible until the cut). Fixes: new fill_small_holes (border-flood, fills interior holes < filter_speckle², gated to non-sketch modes, preserves letter counters); a real shoelace area filter; adaptive preview scale (full-res for normal images, commit ALWAYS uncapped) + scale-normalized thresholds for true WYSIWYG; contour-counting path_count (badge now signals artifacts); enlarged + zoomable trace preview (reused DitherPreviewDialog pattern, top-left origin + overflow:auto for corner reach). Razor PASS (1 WARNING fixed: ignore_area floor), Jen PASS (7 concerns applied). Rust 96→104 tests, JS 628 green; tsc/clippy(no new)/build clean. PENDING OWNER HARDWARE TEST (the burn) for v0.8.21 AND the trace fix."
next: Owner-vetting + hardware-gated tuning. (The trace fix is on the session branch — it needs a v0.8.22 release tag to reach the owner's installed app; first-Start + SVG are already live in v0.8.21.)
  • OWNER HARDWARE TEST — NOW INSTALLABLE in v0.8.21: first-Start freeze (keep-awake fix), SVG import (keyrack). Still pending from earlier builds: material test (presets + text labels + cut border + Frame), text tool (click/type/double-click), slider drag, measure tool, unit toggle (mm/in), aspect ratio lock.
  • IMAGE TRACE — ✅ artifact fix DONE (relay kerf-trace-artifacts, on session branch, awaiting v0.8.22 tag): interior-pinhole despeckle + real area filter + WYSIWYG zoomable preview. Owner test: trace a clean black-on-white PNG with the 'auto' preset, zoom the enlarged preview to confirm no interior speckle, spot-check that an O/e counter still cuts its hole, then burn. SVG PATH-COORDINATE DRIFT remains OPEN — separate bug, awaiting a sample Inkscape SVG from the owner to reproduce.
