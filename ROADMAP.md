---
status: active
current: Save fix (relay kerf-save-fix) — v0.8.11. Day-one mis-permissioning in Tauri ACL (fs:allow-write only granted the low-level write-handle command, not write_text_file — so every save/export/autosave was silently denied). Fix A: replaced capability grants with the correct command set (fs:allow-write-text-file + read-text-file + read-file + stat + remove + mkdir + exists, scope-home-recursive + scope-appdata-recursive). Fix B: saveToPath now returns a boolean and surfaces errors via F26 pattern (setStatusMessage + addConsoleLine); saveProject/saveProjectAs only clear recovery file and add to recent-files on confirmed success; checkUnsavedChanges aborts New/Open if the triggered save fails (was: always proceeded, discarding unsaved work); exportSvg/exportGcode wrapped in try/catch with success info-line + F26 on failure; autoSave mkdir before first write + console.error on failure (interval stays alive). Tests 497→510 JS; tsc/build clean. Owner smoke test required (open Kerf, draw something, Ctrl+S, confirm .kerf file on disk, reopen via Open Recent, export SVG + G-code, wait ~60s, force-quit, relaunch, confirm recovery offer). Base: v0.8.10.
