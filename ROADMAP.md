---
status: active
current: Viewport Tools — v0.8.13 (relay kerf-viewport-tools). Three-part bundle from the 2026-06-21 triage: (a) Measure tool — point-to-point distance + angle readout and ellipse/circle hole-diameter (Ø / W×H), snapping to object anchors/edges/centers (src/lib/measure.ts + tools/toolHandler.ts; toolbar entry + shortcut); (b) Pan/Hand tool (Issue 4) — toolbar tool + hold-Spacebar transient pan over the Pixi stage; (c) live cursor-position (mm) readout in the status bar (StatusBar.tsx ← Viewport setCursorPosition, rounded 0.01 mm). Release-close during power-failure recovery (2026-06-23): the feat commit (40843b2) had been left un-released, and verification caught a build-breaking unused import — formatMeasureLabel (TS6133 under noUnusedLocals) in measure.test.ts — that would have failed the tag build; resolved properly by adding the missing formatMeasureLabel unit test (the function ships live in Viewport.tsx). Also corrected tauri.conf.json version drift (it had been stuck at 0.8.10 across v0.8.11/v0.8.12, so the built app under-reported its version) → now 0.8.13. Verified: tsc --noEmit clean, lint 0 errors, 567 JS tests pass across 40 files; Rust untouched since v0.8.12. Owner test: pick Measure, click two points → distance + angle; hover a circle → diameter; pick Hand (or hold Space) → pan; watch the status-bar mm readout track the cursor. Base: v0.8.12.
next: Still-open from the 2026-06-21 triage — Issue 5 (material-test dialog: verify the power×speed grid generates valid G-code end-to-end in the mm/min unit and can stream to the laser; this is the laser-characterization enabler); Issues 2+3 (move-vs-deform / rotation handle — LIKELY already fixed by the v0.8.10 resize/rotation rework; needs an owner visual drag-pass to confirm and close). Then Fill Phase C — Fix-2 winding hygiene (audit/retire the now-moot CCW import normalization); deferred — cross-hatch in maskFill (B4, only if a post-M4 re-burn still shows grain), offsetFill compound correctness via Clipper2/i_overlay (no new dep yet); then v0.9 — Camera & Rotary. Carry-overs: extract the Rust fill-before-line partition to a shared pub(crate) fn; fillLine-perimeter kerfOffset decision; per-machine scanningOffset calibration; finer rotation snap if wanted. Deferred (owner decision 2026-06-22): revisit fs scope tightening (runtime-persisted dialog scope) post-v0.9 — current home-recursive scope is deliberate for a sole-user desktop tool but narrower scope via dialog API is the right long-term posture.
testing: null
pinned: true
shipped:
  - date: 2026-06-23
