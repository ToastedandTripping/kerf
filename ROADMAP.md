---
status: active
current: "v0.8.17 — Engraving evenness + UX bundle (2026-06-24). HEADLINE = the research-backed fix for edge/feature over-burn ('deep around the letters', edges deeper than center). Root cause (deep-research `laser-raster-fill-evenness-20260624`, 39 sources, adversarially critiqued, PASS): (1) `scan_mask_to_gcode` fired PER RUN (G0 + accel-overscan + engrave + M5 + decel-overscan per run, G0 between runs) so EVERY interior feature boundary got a stop/restart → over-burn at both edges; (2) `$32=0` — Kerf was warn-only and NEVER set it — made M4 a NO-OP (≡ M3, gated on BITFLAG_LASER_MODE in GRBL), exactly the owner's 'M4 barely perceptible' tell. THREE LEVERS: (L2) continuous per-row sweep rewrite — one G0 per row, power cmd (M3/M4) ONCE, S modulated across runs (S=s_max over fills, S0 over gaps), NO M5 and NO interior G0, so the head never decelerates at an interior boundary; (L1) `$32=1` enforcement — an 'Enable Laser Mode' button + red banner in MachinePanel + widened job-gen warning (fires for any raster/fill job); (L3) overscan derived from v²/(2·$120) = clamp(1.2·v²/(2·accelX),3,50)mm replacing the flat 5mm (8× undersized at 200mm/s), fallback accelX=300. Razor PASS (0 CRITICAL/WARNING; golden-snapshot coords independently re-verified incl. 45° rotation; forward+reverse rows + grayscale reverse-pixel indexing preserved). RESOLVES the WS5 engrave-evenness item (and supersedes its M3-flip hypothesis — M4 stays correct; the problem was $32 + firing structure). ALSO BUNDLED this session: drag-overlay Files-gate (dragging knobs/sliders no longer raises the import overlay — App.tsx onDragOver/onDrop gate on dataTransfer 'Files'); engrave speed caps at the X-axis feed ($110) not min(X,Y) so engraving can reach ~12000 if firmware allows (cuts stay conservative); consistent collapse arrows (▶ collapsed / ▼ expanded everywhere via CollapsibleSection); top-level Home button in the laser-control action row. Rust 93→94 tests, JS +12 (overscan/gcodeGen/drag/speedScale); tsc/clippy -D warnings/build all clean. OWNER TEST BURN (the real gate, hardware-affecting): Enable Laser Mode (or use the new banner), then re-burn the white-on-black letters + a solid fill — expect a FLAT surface, edges no deeper than the centre. Base: v0.8.16."
next: The software revamp (gentle-humming-steele.md) is COMPLETE through Phase 3; what remains is owner-vetting + hardware-gated tuning + one small material-test relay.
  ...
  • WS6 INTERRUPT-RECOVERY — bound the streaming pump with a stall-vs-progress deadline (GRBL `?` status replies currently reset the 60s liveness timer forever, so a sleep-eaten `ok` wedges the pump indefinitely holding the command lock) + clean abort (0x18 + drain + recovery UI), all laser-off. The deadline TUNE needs the owner's laser (a healthy slow line must not abort) → bench session with a live slow cut.
