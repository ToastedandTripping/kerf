...
- **Engine arms every real job with positive S on standalone mode lines (S2 Deferral 1): X1-class on every M3 layer.** A stationary beam at every path start on any layer set to M3. QUEUED as the batch after S3 (coordinator, 2026-09-25, under Lee's technical delegation): plan `.claude/plans/kerf-safety-engine-arm.md`, with golden-regeneration discipline. It is in today's build v0.8.30 on every layer set to constant power (M3). See `### Deferred from kerf-safety-s2 (2026-09-25)` below.
