---
status: active
current: Sidebar UX rebuild — v0.8.16 (relay kerf-sidebar-rebuild; Phase 3 of the cut-reliability/UX revamp — the LAST software phase; the revamp's software work WS1-WS4+WS2+WS3 is now all shipped). WS3 rebuilt the right sidebar into a workflow-weighted three-region layout: a pinned ACTIVE-LAYER STRIP at the top (always-visible Power slider + Speed for the active layer — the most-touched controls were 3 clicks deep), a pinned JOB ACTION BAR at the bottom (START/FRAME/PAUSE/STOP + progress, always reachable — was buried in a capped panel), and a scrollable middle reordered to the job flow (Layers → Machine → Materials → Properties). The 40% MachinePanel cap is gone; jog moved into a collapse-by-default 'Positioning' section. Extracted JobActionBar + ActiveLayerStrip as new components, promoted CollapsibleSection to its own file, plus legibility (layer power/speed VALUES → text-primary, header hierarchy, hover states, --panel-width 280→300). Razor PASS (0 CRITICAL; React Error 185-safe — every new useStore site reads scalars/stable-refs, active layer derived OUTSIDE the selector; handler MOVE verified verbatim — START/FRAME/PAUSE/STOP logic + gate unchanged, no dangling refs; re-pointed job-loop tests exercise the real loop, not stubs; 1 WARNING [pre-existing jobProgress stale closure] + 1 NOTE [missing speed test] both closed). Jen design review PASS with 3 CONCERNs applied (strip speed → bare number field, START gets a border to read as primary, black-layer swatch gets an inset outline). Tests 592→597 JS; tsc/lint/build clean. **OWNER VISUAL PASS when testing:** strip reads as a compact HUD not a mini-panel; START draws the eye over FRAME; black Layer-0 swatch visible; sidebar scroll pins strip+bar at min window height; collapsible-header hover. Base: v0.8.15.
next: The software revamp (gentle-humming-steele.md) is COMPLETE through Phase 3; what remains is owner-vetting + hardware-gated tuning + one small material-test relay.
  ...
  • WS5 ENGRAVE-EVENNESS — the immediate test is a MANUAL A/B (no code): duplicate real artwork (text + a solid fill) side by side, set one layer to Constant (M3) / the other to Variable (M4) via the LayerPanel → expand layer → 'Power Mode' dropdown, same power/speed, burn both and compare. A rectangle grid will NOT reveal it — the M4 stroke-length under-power only shows on short/thin strokes (serifs, counters, letter detail), where the head never reaches feed. Pre-checks: confirm the G-code shows `M4` + `; Mask Fill:` + a constant S per sweep; `$30` == Kerf's S-Value-Max (M4 scales power as a fraction of $30 — a mismatch alone breaks it); `$32=1`. If M3 reads more even on fine detail → scoped DEFAULT FLIP to M3 for binary maskFill text (keep M4 for true grayscale images).
