---
status: active
current: Cut reliability — v0.8.14 (relay kerf-keepawake-firststart; Phase 1a of the cut-reliability/UX revamp, plan .claude/plans/gentle-humming-steele.md). Two fixes that stop cuts from dying. WS1 — first-Start no-move: the GRBL startup-banner read loop left a stale serial fragment on a read-timeout (serial.rs `Err(_) => break` without clearing `pending`), so the FIRST Start's response misparsed as a reset banner and aborted — laser didn't move; Stop+Start worked because the soft-reset flushed the buffer. Fixed by extracting `drain_startup_banner` (pub(crate)) which guarantees `pending` is empty on every exit path (timeout / EOF / Grbl-line); the regression test now calls production (not a shadow loop) + an EOF-path test. WS4 — keep-awake during jobs: a sleeping computer paused the serial stream and halted the cut; Kerf now holds a system sleep-inhibitor (keepawake =0.6.0 — native per-OS RAII: macOS IOKit, Linux logind/ScreenSaver, Windows SetThreadExecutionState) for the full duration of any running job, driven by ONE app-lifetime useStore.subscribe on the scalar `jobRunning` (covers BOTH the main job and the material-test grid via the shared flag; both invokes .catch-swallowed so a power-assertion failure never breaks a job; idempotent acquire/release). Razor PASS (React Error 185-safe — full-state subscribe, no new-object selector; laser-safe — zero motion/laser/G-code in the path; no leak — all 5 jobRunning→false sites release). Tests 91→93 Rust + 567→574 JS; tsc/lint/clippy -D warnings clean. **OWNER HARDWARE TEST:** (1) connect laser, hit Start ONCE → laser moves on the first attempt (no Stop+Start dance); (2) start a job, run `pmset -g assertions` → a Kerf/PreventUserIdleSystemSleep assertion is present during the job and GONE after Stop/complete. Base: v0.8.13.
next: Continue the phased revamp (plan gentle-humming-steele.md). Phase 1 remainder — WS6 interrupted-job recovery: bound the streaming pump with a stall-vs-progress deadline (GRBL `?` status replies currently reset the 60s liveness timer forever, so a sleep-eaten `ok` wedges the pump indefinitely holding the command lock) + clean abort (0x18 + drain + recovery UI), all laser-off; the deadline tune needs owner hardware (a healthy slow line must not abort). Phase 2 — WS2 strict layer-order for ALL ops (raster images currently emit first, ignoring layer position) + a non-blocking cut-above-engrave warning + the device-name overflow fix. Phase 3 — WS3 sidebar UX rebuild (pinned active-layer Power/Speed strip, fixed START/FRAME/STOP action bar, reorder, collapse defaults). WS5 (parallel hardware track) — engrave-evenness A/B burn: M4-on-binary-maskFill is the prime regression suspect (short glyph runs under-power); owner runs the test matrix, then a scoped M3-default flip if implicated. Older carry-overs (Issue 5 material-test verify; Issues 2+3 visual drag-pass; Fill Phase C winding hygiene; v0.9 Camera & Rotary) remain after the revamp.
testing: null
pinned: true
shipped:
  - date: 2026-06-23
