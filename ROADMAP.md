...
next: "REMEDIATION Phase 1 IN PROGRESS — Batch B1 (backend admission fence + reset submission) COMPLETE (relay kerf-b1-admission-fence, Ted+Razor PASS, 2026-09-21). SerialSession state machine: connection epoch, admitted job phases (idle/active/stopping/unknown/disconnected), generation-checked submission permits (try_permit protocol), native stop command (0x18 immediately per DECISIONS ruling — no hold, no M5, no ack wait), banner publication (any reader body that sees Banner while stop_in_flight publishes it), per-line admission (serial_job_begin/serial_job_end), ScriptedPort harness additions (hold points, session-event observer, real deadline). 281 Rust tests (21 new), 8/8 mutants killed. Razor PASS after 2 WARNING fixes (banner_observed clear ordering, dead test removal). NEXT BATCHES: B2a (native status contract) + B2b (TS status consumer), B3 (job lifetime through draining), B4 (shared abort/disconnect + pause containment). Abort policy ruling (2026-09-20) unblocked all Phase 1 batches. RELEASE BLOCKED — two hardware-confirmed safety defects remain (pause re-arm, laser-switch wedge). Phase 2A A/B test (gate D1c) deferred behind both. Batch 2.3 must be re-specified (restricted envelope → full feature set corrections)."
