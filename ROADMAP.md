---
status: active
current: "v0.8.18 — First-Start freeze fix (2026-06-24). Root cause (audited + 3 independent code probes + owner hardware-confirmed): the FIRST job of a session froze a fraction of a second into the initial travel move (laser off, head 'paused'); only the first Stop (0x18) cured it, then every Start worked. connect() NEVER soft-reset GRBL, so on serial adapters that don't reset GRBL on port-open (no DTR pulse) the controller began the session in stale parser/modal state, the first stream desynced, and the streaming pump (run_pump) WEDGED on a lost 'ok' — because every '?' liveness probe was answered with a <…> status reply that reset the silence timer, so it waited forever holding the command lock. FRAME and HOME were red herrings — generate→Start alone reproduced it (owner confirmed). The prior 'first-Start' patches (v0.8.9 alarm gate, v0.8.14 banner-drain) fixed adjacent failures, not this missing-reset one. FIX: (1) connect() now sends 0x18 + a 2s settle before $$ so every session starts on a clean known GRBL state (standard GRBL-sender practice; on $22=1 machines the resulting Alarm is already handled — owner homes to clear); (2) run_pump parses machine state from <State|…> and, after 3 consecutive Idle status replies with no terminal, returns a recoverable laser-OFF stall instead of waiting forever (Run never trips it; threshold owner-tunable — closes the WS6 liveness wedge); (3) the disconnect/stall path tears down the port after the M5 safety volley so reconnect starts clean (was: 'port busy'). Razor PASS (laser-off path traced through the M5 volley; 3-tick≈3s threshold won't false-trip a healthy cut or the v0.8.17 continuous sweep, which stays in Run). Rust 94→96 tests, JS connection +1; tsc/clippy/build clean. OWNER TEST: connect (watch for 'Soft reset on connect' in the console) → home → Start — it should run on the FIRST try now. Base: v0.8.17. ⚠ OWNER-TESTED 2026-06-25: this did NOT fix it — still freezes on the first Start in EVERY combination; the idle-stall safety net DID fire (auto-disconnect after ~few seconds, which is new+correct). RE-OPENED — see the FIRST-START FREEZE item in `next`. [Prior: v0.8.17 shipped the engraving-evenness fix — continuous per-row sweep + $32=1 enforcement + v²/2a overscan — plus the drag-overlay / X-axis-speed-cap / collapse-arrows / Home-button bundle; see the v0.8.17 shipped entry. Engraving burn result still owner-pending.]"
next: The software revamp (gentle-humming-steele.md) is COMPLETE through Phase 3; what remains is owner-vetting + hardware-gated tuning + one small material-test relay.
  • OWNER VETTING — a 36-item shipped-feature test checklist (v0.8.6→v0.8.16, prioritized P0 data-loss/safety → P1 reliability → P2 physical burns → P3 interaction/UI) at `~/marvin/state/kerf-test-checklist.html` (interactive, persists, printable). Run before more lands. Pending smoke: first-Start STILL FREEZES first try (v0.8.18 soft-reset-on-connect did NOT fix it — owner-confirmed 2026-06-25; the idle-stall safety net DID fire + auto-disconnect; RE-OPENED — see FIRST-START FREEZE item below), pmset keep-awake during a job (v0.8.14), cut-order follows layers + cut-above-engrave warning (v0.8.15), sidebar visual pass — strip/START-weight/black-swatch/scroll-pin (v0.8.16), and the move-vs-deform + rotation-handle drag-pass to CLOSE Issues 2+3.
  • WS5 ENGRAVE-EVENNESS — ✅ ADDRESSED in v0.8.17 (research `laser-raster-fill-evenness-20260624` → continuous per-row sweep + `$32=1` enforcement + v²/2a overscan). The earlier 'M4 under-powers short strokes → maybe DEFAULT-FLIP to M3' hypothesis is SUPERSEDED: the definitive diagnosis is `$32=0` (M4 was a literal no-op, ≡ M3) + the per-run G0/M5 firing structure — NOT an M3-vs-M4 choice; M4 stays correct (constant energy/mm through ramps). OWNER TEST BURN validates: Enable Laser Mode, re-burn white-on-black letters + a solid fill, expect flat edges (no deeper than centre). PRE-CHECK still useful: `$30` == S-Value-Max (M4 scales power as a fraction of $30). RESIDUAL WATCH (not yet observed): if short/thin strokes now read too LIGHT — M4 correctly scaling power down where the head never reaches feed — that's the follow-up (a minimum-feature-width advisory, or per-feature dwell), NOT a reason to abandon M4.
  • MATERIAL-TEST RELAY (NEW, surfaced 2026-06-24) — Issue #5 is actually BUILT + wired (menu → Material Test Grid: power×speed grid, min/max/steps, cell size, cut/fill mode, Send-to-Machine with abort-on-error / Copy-G-code, speed capped to machine max) but (a) UNVERIFIED on real hardware and (b) HARDCODES `M4` (MaterialTestDialog.tsx:72,87) — a real flaw: it can't match an M3 layer and can't compare modes. Relay scope: verify #5 end-to-end on the laser + make the grid powerMode-AWARE (use the layer's M3/M4 instead of hardcoded M4) + add a dedicated 'M3 vs M4' comparison that burns a FINE-DETAIL pattern (thin strokes, not rectangles) both ways side by side — the version that actually exposes the difference. Small, well-scoped; queue alongside WS6/WS5.
  • WS6 INTERRUPT-RECOVERY — ✅ CORE DONE in v0.8.18. The liveness wedge is closed: run_pump now parses `<State|…>` and, after 3 consecutive Idle-status replies with no terminal, returns a recoverable laser-OFF stall instead of resetting the liveness timer forever on `?` replies (the exact wedge that caused the first-Start freeze); the disconnect/stall path also tears down the port so reconnect starts clean. REMAINING: (a) owner-hardware TUNE of the 3-tick (~3s) idle-stall threshold — a healthy slow line must not abort → bench session with a live slow cut; (b) a friendlier in-app stall-recovery UI (currently it surfaces as a disconnect + requires reconnect); (c) the broader clean-abort/drain UX for user-initiated mid-job interrupts.
  • FIRST-START FREEZE — RE-OPENED (owner-tested 2026-06-25; v0.8.18 did NOT fix it). The soft-reset-on-connect (0x18 before $$) did NOT stop the freeze: the laser still fails to fire on the FIRST Start in EVERY combination (generate→Start, →Home→Start, →Frame→Start). What DID work: the new idle-stall detector + auto-disconnect fired correctly (laser disconnected after ~a few seconds of the failed start) — the WS6 safety net is confirmed-good on hardware, and the wedge mechanism is therefore real and now CAUGHT, but the PREVENTION is wrong. New constraints for the next investigation: (a) CONFIRM the connect 0x18 actually reaches+resets GRBL — look for the reboot banner in the console on connect; if it's absent, the realtime byte isn't landing that early (sendByte swallows its error) and the reset never happened. (b) The clean state likely RE-DIRTIES between connect and the first job — queryGrblSettings ($$) + the 250ms status poll run in between — so the fix probably belongs IMMEDIATELY BEFORE the first job stream (a pre-flight quiesce+drain, the original audit's option 1), NOT at connect. (c) The curing Stop path is `!` feed-hold THEN 0x18 (emergencyStop) while STAYING connected; compare what that leaves vs a bare connect-time 0x18 — the cure may depend on the feed-hold or on post-failed-start GRBL state, not a bare reset. (d) Re-examine whether it's KERF-SIDE pump/channel state (not GRBL state) that only the Stop path clears. Full audit + 4 cold probes are in this session's transcript. Laser-safe (it now fails safe via auto-disconnect), so not blocking — but it blocks reliable first-cut UX and is the top reliability item.
  • SVG IMPORT — PATHS SHIFT / DISJOINT on import (owner-reported 2026-06-25; owner's current priority = crisp CUT lines). Importing an SVG, the OVERALL geometry/bounding box stays correct but PARTS WITHIN it move/shift relative to each other — sub-paths get displaced or disjointed, so a clean vector comes in scrambled internally. Goal: import an SVG and have it remain intact for crisp cutting. Likely a TRANSFORM-COMPOSITION bug in the SVG import path (`src/lib/fileOps/` SVG parse + the compound-path / subpath split): nested/group transforms or per-subpath offsets not composed onto child geometry correctly, OR per-element coordinate-origin drift (prior fortification touched rotated-primitive→path, compound-path splitting, and transform flattening — a regression or gap there is plausible). Start: import a known multi-subpath SVG with group transforms; compare imported anchor coords against the source; check how group/nested transforms flatten onto subpaths and whether the compound-path split preserves each subpath's ABSOLUTE position. This is the front door for crisp cutting — prioritize alongside FIRST-START.
