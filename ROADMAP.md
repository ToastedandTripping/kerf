---
status: active
current: "Post-v0.8.20 (2026-06-26-27). Two long-standing root causes found and fixed: (1) FIRST-START FREEZE: keep_awake_acquire ran synchronously on the Tauri event loop — the keepawake OS API call (~0.5s) blocked async serial reads, tripping the idle-stall detector. Frame worked because it didn't set jobRunning (no keep-awake call). Reconnect worked because the guard was already held (no-op return). Fix: spawn_blocking. Three prior attempts (TS 0x18, DTR toggle, 0x18 fallback) were chasing GRBL state — the issue was never on the controller side. (2) SVG IMPORT: parsePathD treated coordinate pairs after m/M as implicit m/M (more movetos). SVG spec says they must be implicit l/L (lineto). Every path using the common 'm x,y dx1,dy1...' shorthand had its segments split into 1-point fragments and filtered out. Fix: M→L, m→l for implicit continuation. Also shipped: v0.8.19 (text tool, unit toggle, aspect lock, slider/measure fixes), v0.8.20 (material test overhaul with power mode, presets, Frame, text labels via text-to-G-code pipeline, cut border). WS5 engraving evenness CONFIRMED on hardware (owner: M4 engrave came out much cleaner). All pending owner hardware test."
next: Owner-vetting + hardware-gated tuning.
  • OWNER HARDWARE TEST — first-Start freeze (keep-awake fix), SVG import (keyrack), material test (presets + text labels + cut border + Frame), text tool (click/type/double-click), slider drag, measure tool, unit toggle (mm/in), aspect ratio lock.
